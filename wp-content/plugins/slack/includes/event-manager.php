...
class WP_Slack_Event_Manager {
	...
	public function get_events() {
		return apply_filters( 'slack_get_events', array(
			...
			'customer-research' => array(
	        'action'      => 'transition_customer-research',
	        'description' => __( 'When a post type of Customer Research Day is published', 'slack' ),
	        'message'     => function( $new_status, $old_status, $post ) {
			...
			'check-customer-research-category' => array(
	        'action'      => 'transition_post_status',
	        'description' => __( 'When a post with any research category is posted', 'slack' ),
	        'message'     => function( $new_status, $old_status, $post ) {

	        	$isResearchPost = false;

	        	$categoriesToMatch = ['Research','Benchmarking','Customer Advisory Board','Customer Advisory Council','Customer Research Days'];
	        	$postCategories = get_the_category($post->ID);

	        	foreach($postCategories as $postCategory){
	        		if(in_array($postCategory->name, $categoriesToMatch)) {
	        			$isResearchPost = true;
	        		}
	        	}

	        	if ( 'publish' !== $old_status && 'publish' === $new_status && $isResearchPost) {
	        		$excerpt = has_excerpt( $post->ID ) ?
							apply_filters( 'get_the_excerpt', $post->post_excerpt )
							:
							wp_trim_words( strip_shortcodes( $post->post_content ), 55, '&hellip;' );
